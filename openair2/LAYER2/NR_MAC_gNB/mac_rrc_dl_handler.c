...
void allocate_and_initialize_rach_LessHO_r18(NR_ReconfigurationWithSync_t *reconfig) {
      reconfig->ext4 = calloc(1, sizeof(struct NR_ReconfigurationWithSync__ext4));
      reconfig->ext4->rach_LessHO_r18=calloc(1, sizeof(* reconfig->ext4->rach_LessHO_r18));
      reconfig->ext4->rach_LessHO_r18->targetNTA_r18 = (long *)malloc(sizeof(long));
      asn1cCallocOne(reconfig->ext4->rach_LessHO_r18->targetNTA_r18, NR_RACH_LessHO_r18__targetNTA_r18_zero);
}
void allocate_and_initialize_NR_cg_RRC_Configuration_r18(NR_UplinkConfig_t *uplinkConfig) {
    //NR_UplinkConfig_t *uplinkConfig = NULL;
    //uplinkConfig = calloc(1, sizeof(struct NR_UplinkConfig));
    if(!uplinkConfig->initialUplinkBWP )
      uplinkConfig->initialUplinkBWP = calloc(1, sizeof(struct NR_BWP_UplinkDedicated));
      
    NR_SetupRelease_ConfiguredGrantConfig_t *configuredGrantConfig = NULL;
    

    // Allocate memory for the structure
    asn1cCalloc(configuredGrantConfig, NR_SetupRelease_ConfiguredGrantConfig_t);
    ...
    configuredGrantConfig->present = NR_SetupRelease_ConfiguredGrantConfig_PR_setup;

    // Allocate memory for the setup choice
    asn1cCalloc(configuredGrantConfig->choice.setup, NR_ConfiguredGrantConfig_t);
    if (configuredGrantConfig->choice.setup == NULL) {
        printf("Memory allocation for setup failed\n");
        free(configuredGrantConfig);
        return NULL;
    }

    // Initialize optional fields to NULL
    configuredGrantConfig->choice.setup->frequencyHopping = NULL;
    configuredGrantConfig->choice.setup->mcs_Table = NULL;
    configuredGrantConfig->choice.setup->mcs_TableTransformPrecoder = NULL;
    configuredGrantConfig->choice.setup->uci_OnPUSCH = NULL;
    configuredGrantConfig->choice.setup->rbg_Size = NULL;
    configuredGrantConfig->choice.setup->transformPrecoder = NULL;
    configuredGrantConfig->choice.setup->repK_RV = NULL;
    configuredGrantConfig->choice.setup->configuredGrantTimer = NULL;

    // Initialize mandatory fields with reasonable values
    configuredGrantConfig->choice.setup->resourceAllocation = 1; // Type 1 allocation
    configuredGrantConfig->choice.setup->powerControlLoopToUse = 1; // Assume simple power control
    configuredGrantConfig->choice.setup->p0_PUSCH_Alpha = 0; // Example value (adjust based on target SNR)
    configuredGrantConfig->choice.setup->nrofHARQ_Processes = 8; // Common number of HARQ processes
    configuredGrantConfig->choice.setup->repK = 0; // No repetition
    configuredGrantConfig->choice.setup->periodicity = 1; // Periodicity in ms
    LOG_I(NR_MAC, "configuredGrantConfig->choice.setup->periodicity = %ld\n", configuredGrantConfig->choice.setup->periodicity);
    // Initialize nested structures if necessary
    asn1cCalloc(configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant, NR_ConfiguredGrantConfig__rrc_ConfiguredUplinkGrant);
    if (configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant == NULL) {
        printf("Memory allocation for rrc_ConfiguredUplinkGrant failed\n");
        free(configuredGrantConfig->choice.setup);
        free(configuredGrantConfig);
        return NULL;
    }

    // Example initialization for rrc_ConfiguredUplinkGrant
    configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->timeDomainOffset = 13; // Example value, adjust as needed
    configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->timeDomainAllocation = 0; // Mapping Type A/B

    // Initialize frequencyDomainAllocation (BIT_STRING)
    configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->frequencyDomainAllocation.buf = (uint8_t *)calloc(1, sizeof(uint8_t));
    if (configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->frequencyDomainAllocation.buf == NULL) {
        printf("Memory allocation for frequencyDomainAllocation failed\n");
        free(configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant);
        free(configuredGrantConfig->choice.setup);
        free(configuredGrantConfig);
        return NULL;
    }
    configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->frequencyDomainAllocation.size =  1;
    configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->frequencyDomainAllocation.buf[0] = 0b11111000; // Full bandwidth allocation
    configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->frequencyDomainAllocation.bits_unused = 3;

    // Continue initializing other mandatory fields of rrc_ConfiguredUplinkGrant
    configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->antennaPort = 0; // Default antenna port
    configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->precodingAndNumberOfLayers = 1; // Single layer
    configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->mcsAndTBS = 7; // Example MCS
    configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->pathlossReferenceIndex = 0; // Default pathloss reference

    // Extend to include NR_ConfiguredGrantConfig__rrc_ConfiguredUplinkGrant__ext3
    asn1cCalloc(configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->ext3, NR_ConfiguredGrantConfig__rrc_ConfiguredUplinkGrant__ext3);
    if (configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->ext3 == NULL) {
        printf("Memory allocation for rrc_ConfiguredUplinkGrant__ext3 failed\n");
        free(configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->frequencyDomainAllocation.buf);
        free(configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant);
        free(configuredGrantConfig->choice.setup);
        free(configuredGrantConfig);
        return NULL;
    ...
    // Example initialization for ext3
    configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->ext3->srs_ResourceSetId_r18 = NULL;
    configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->ext3->cg_LTM_Configuration_r18 = NULL;
    configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->ext3->cg_SDT_PeriodicityExt_r18 = NULL;
    configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->ext3->timeReferenceHyperSFN_r18 = NULL;
    configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->ext3->applyIndicatedTCI_State_r18 = NULL;

    // Allocate and initialize cg_RRC_Configuration_r18
    configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18 = (NR_CG_RRC_Configuration_r18_t *)calloc(1, sizeof(NR_CG_RRC_Configuration_r18_t));
    if (configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18 == NULL) {
        printf("Memory allocation for cg_RRC_Configuration_r18 failed\n");
        free(configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->ext3);
        free(configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->frequencyDomainAllocation.buf);
        free(configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant);
        free(configuredGrantConfig->choice.setup);
        free(configuredGrantConfig);
    ...
    // Initialize cg_RRC_Configuration_r18 fields
    configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18->cg_RRC_RetransmissionTimer_r18 = (long *)calloc(1, sizeof(long));
    if (configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18->cg_RRC_RetransmissionTimer_r18 != NULL) {
        *(configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18->cg_RRC_RetransmissionTimer_r18) = 8; // Example value, adjust as needed
    }

    configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18->cg_RRC_RSRP_ThresholdSSB_r18 = (NR_RSRP_Range_t *)calloc(1, sizeof(NR_RSRP_Range_t));

    if (configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18->cg_RRC_RSRP_ThresholdSSB_r18 != NULL) {
          *(configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18->cg_RRC_RSRP_ThresholdSSB_r18) = 30; // Example value
      }

    // Initialize mcs_TableTransformPrecoder (設定具體的 Transform Precoding MCS 表)
    long *mcsTable = (long *)malloc(sizeof(long));
    if (mcsTable == NULL) {
        printf("Memory allocation for mcsTable failed\n");
        return NULL;
    }
    
    // Example setting: 根據具體需求設置
    *mcsTable = 1;  // 例如：1 代表使用 QPSK，具體參數可根據規範來設置
    configuredGrantConfig->choice.setup->mcs_TableTransformPrecoder = mcsTable;
    uplinkConfig->initialUplinkBWP->configuredGrantConfig=configuredGrantConfig;
...
static NR_UE_info_t *create_new_UE(gNB_MAC_INST *mac, uint32_t cu_id)
{
  ...
  allocate_and_initialize_rach_LessHO_r18(UE->CellGroup->spCellConfig->reconfigurationWithSync);
  allocate_and_initialize_NR_cg_RRC_Configuration_r18(UE->CellGroup->spCellConfig->spCellConfigDedicated->uplinkConfig);
