...
static void nr_generate_rach_less(module_id_t module_idP,
                             int CC_id,
                             frame_t frameP,
                             sub_frame_t slotP,
                             NR_RA_t *ra,
                             ...
                             nfapi_nr_tx_data_request_t *TX_req)
{
  ...
  // no DL -> cannot send rach-less
  // LOG_I(NR_MAC, "frame:%d, slot:%d, UE %04x: generate rach-less\n", frameP, slotP, ra->rnti);
  if (!is_xlsch_in_slot(nr_mac->dlsch_slot_bitmap[slotP / 64], slotP)) {
    return;
  }
  int c=0;
  ...
  NR_ControlResourceSet_t *coreset = ra->coreset;
  AssertFatal(coreset, "Coreset cannot be null for RA-less\n");
  const int coresetid = coreset->controlResourceSetId;
  // Calculate number of symbols
  int time_domain_assignment = get_dl_tda(nr_mac, scc, slotP);
  int mux_pattern = type0_PDCCH_CSS_config ? type0_PDCCH_CSS_config->type0_pdcch_ss_mux_pattern : 1;
  ...
  if (CCEIndex < 0) {
    LOG_W(NR_MAC, "UE %04x: %d.%d cannot find free CCE for RACH-less!\n", ra->rnti, frameP, slotP);
    return;
  }

  LOG_D(NR_MAC, "RACH-less startSymbolIndex.nrOfSymbols %d.%d\n", tda_info.startSymbolIndex, tda_info.nrOfSymbols);
  ...
  LOG_A(NR_MAC,
        "UE %04x: %d.%d Generating RA-less DCI, state %d, CoreSetType %d, RAPID %d\n",
        ra->rnti,
        frameP,
        slotP,
        ra->ra_state,
  ...
  LOG_I(NR_MAC, "RACH-less rbSize.rbStart.BWPsize %d.%d.%ld\n", pdsch_pdu_rel15->rbSize, pdsch_pdu_rel15->rbStart, BWPSize);
  ...
  if (ra->cfra) {
    ...
    if (UE) {
      ...
      nr_timer_setup(&sched_ctrl->transm_interrupt, delay_ms << ra->DL_BWP.scs, 1);
      nr_timer_start(&sched_ctrl->transm_interrupt);
    }
  }
                    ...
                    nfapi_nr_tx_data_request_t *TX_req)
{
  ...
  for (int CC_id = 0; CC_id < MAX_NUM_CCs; CC_id++) {
    ...
    for (int i = 0; i < NR_NB_RA_PROC_MAX; i++) {
      ...
      if (ra->ra_type == RA_4_STEP && ra->ra_state >= nrRA_WAIT_Msg3 && ra->ra_state != nrRA_less) {
        ra->contention_resolution_timer--;
...
void nr_schedule_RA_less(module_id_t module_idP,
                    frame_t frameP,
                    sub_frame_t slotP,
                    nfapi_nr_ul_dci_request_t *ul_dci_req,
                    nfapi_nr_dl_tti_request_t *DL_req,
  ...
  for (int CC_id = 0; CC_id < MAX_NUM_CCs; CC_id++) {
    ...
    for (int i = 0; i < NR_NB_RA_PROC_MAX; i++) {
      ...
      switch (ra->ra_state) {
        case nrRA_less:
          //configured_grant
          nr_generate_rach_less(module_idP, CC_id, frameP, slotP, ra, DL_req, TX_req);
          //ra->rach_less_timer = 1;
          //cc->sync_state=1;
          break;
        default:
          break;
