...
bool is_configured_grant_slot(int current_SFN, int current_slot, int current_symbol, int periodicity, int timeReferenceSFN, int timeDomainOffset, int S, int numberOfSlotsPerFrame, int numberOfSymbolsPerSlot) {
    ...
    LOG_D(NR_MAC, "current_SFN: %d, current_slot: %d, current_symbol: %d\n", current_SFN, current_slot, current_symbol);
...
void nr_schedule_ulsch(module_id_t module_id, frame_t frame, sub_frame_t slot, nfapi_nr_ul_dci_request_t *ul_dci_req)
{
  ...
  /*
    ...
    LOG_I(NR_MAC, "UL scheduling for frame %d, slot %d\n", frame, slot);
  */
  ...
  UE_iterator( UE_info->list, UE) {
    ...
    NR_SetupRelease_ConfiguredGrantConfig_t *configured_grant_setup = UE->CellGroup->spCellConfig->spCellConfigDedicated->uplinkConfig->initialUplinkBWP->configuredGrantConfig;
    NR_ConfiguredGrantConfig_t *configured_grant=NULL;
    if(configured_grant_setup) configured_grant=configured_grant_setup->choice.setup;
    ...
    * because we have to schedule the DCI0 first before schedule uplink data */
    ...
    if (false){ //configured_grant
      //LOG_I(NR_MAC, "Enter configured grant\n");
      int period=(int)configured_grant->periodicity;
      //LOG_I(NR_MAC, "periodicity %d\n", period);
      bool is_valid_slot = is_configured_grant_slot(frame, slot, 0, period, 20,
                                                            4, 0,
                                                            nr_slots_per_frame[current_BWP->scs], 14 );
      if (is_valid_slot) LOG_D(NR_MAC, "Configured Grant: UL_TTI.request created for frame %d, slot %d\n", frame, slot);
    ...
    if (false){ //configured_grant
      LOG_D(NR_MAC,"Scheduled UE %04x for configured grant in %d.%d\n",rnti,sched_pusch->frame,sched_pusch->slot);
      LOG_D(NR_MAC,"Start symbol: %d, Number of symbols: %d\n",sched_pusch->tda_info.startSymbolIndex,sched_pusch->tda_info.nrOfSymbols);
    }
    LOG_D(NR_MAC,
    ...
    /*
    if(&cc->CG.start_frame==0 || &cc->ra->rach_less_timer!=0)
      &cc->CG.start_frame=frame;
    */
    ...
    //LOG_I(NR_MAC,"ULSCH/PUSCH: %d.%d RNTI %04x, coreset %d, ss %d\n", frame, slot, rnti, coresetid, ss->searchSpaceId);
    ...
    /*0918 Configured both on gNB and UE */
    /*===========================*/
    if (false) { //configured_grant
      pusch_pdu-> bwp_size= 106;
      pusch_pdu-> bwp_start= 0;
      pusch_pdu-> subcarrier_spacing= 1;
      pusch_pdu-> cyclic_prefix= 0;
      pusch_pdu-> target_code_rate= 6790;
      pusch_pdu-> qam_mod_order= 2;
      pusch_pdu-> mcs_index= 6;
      pusch_pdu-> mcs_table= 0;
      pusch_pdu-> transform_precoding= 1;
      pusch_pdu-> scid= 0;
      pusch_pdu-> data_scrambling_id= 1;

      pusch_pdu-> ul_dmrs_scrambling_id= 1;
      pusch_pdu-> nrOfLayers= 1;
      pusch_pdu-> num_dmrs_cdm_grps_no_data= 1;
      pusch_pdu-> dmrs_ports= 1;
      pusch_pdu-> ul_dmrs_symb_pos= 1057;
      ...
      pusch_pdu-> nr_of_symbols= 13;




      // Assuming the frequencyDomainAllocation bit string is already configured as in the previous example
      ...
      pusch_pdu->rb_start = rb_start;
      pusch_pdu->rb_size = rb_size;
      
      pusch_pdu-> start_symbol_index= (uint8_t)configured_grant->rrc_ConfiguredUplinkGrant->timeDomainAllocation; //0
      pusch_pdu-> nr_of_symbols= (uint8_t)configured_grant->rrc_ConfiguredUplinkGrant->timeDomainOffset ;  //13
      
      pusch_pdu-> pusch_data.rv_index= 0;
      pusch_pdu-> pusch_data.new_data_indicator= 1;
      pusch_pdu-> pusch_data.tb_size= 88;
      pusch_pdu->dfts_ofdm.low_papr_group_number=0;     // Low PAPR group number
      pusch_pdu->dfts_ofdm.low_papr_sequence_number=0; // Low PAPR sequence number
      pusch_pdu->pusch_ptrs.ptrs_freq_density=0;        // PTRS frequency density
      pusch_pdu->pusch_ptrs.ptrs_time_density=0;        // PTRS time density
      pusch_pdu->pusch_ptrs.num_ptrs_ports=0;           // Number of PTRS ports
      pusch_pdu->pusch_ptrs.ul_ptrs_power=0;            // UL PTRS power
      pusch_pdu->pusch_data.num_cb=0;                   // Number of code blocks
      pusch_pdu->pdu_bit_map=1;                          // PDU bitmap
    }
    
    
    LOG_D(NR_MAC,
      "PUSCH Config PDU Information: "
      "rnti: %x, bwp_size: %d, bwp_start: %d, subcarrier_spacing: %d, cyclic_prefix: %d, "
      "target_code_rate: %d, qam_mod_order: %d, mcs_index: %d, mcs_table: %d, "
      "transform_precoding: %d, scid: %d, data_scrambling_id: %d, ul_dmrs_scrambling_id: %d, "
      "nrOfLayers: %d, num_dmrs_cdm_grps_no_data: %d, dmrs_ports: %d, ul_dmrs_symb_pos: %u, "
      "dmrs_config_type: %d, resource_alloc: %d, rb_start: %d, rb_size: %d, "
      "frequency_hopping: %d, vrb_to_prb_mapping: %d, start_symbol_index: %d, nr_of_symbols: %d, "
      "pusch_data.rv_index: %d, pusch_data.harq_process_id: %d, pusch_data.new_data_indicator: %d, "
      "pusch_data.tb_size: %d, tbslbrm: %d, low_papr_group_number: %d, low_papr_sequence_number: %d, "
      "ptrs_freq_density: %d, ptrs_time_density: %d, num_ptrs_ports: %d, ul_ptrs_power: %d, "
      "absolute_delta_PUSCH: , ldpcBaseGraph: , num_cb: %u, pdu_bit_map: %u\n",
      pusch_pdu->rnti,                                // RNTI
      pusch_pdu->bwp_size,                            // BWP size
      pusch_pdu->bwp_start,                           // BWP start
      pusch_pdu->subcarrier_spacing,                  // Subcarrier spacing
      pusch_pdu->cyclic_prefix,                       // Cyclic prefix
      pusch_pdu->target_code_rate,                    // Target code rate
      pusch_pdu->qam_mod_order,                       // QAM modulation order
      pusch_pdu->mcs_index,                           // MCS index
      pusch_pdu->mcs_table,                           // MCS table
      pusch_pdu->transform_precoding,                 // Transform precoding
      pusch_pdu->scid,                                // SCID
      pusch_pdu->data_scrambling_id,                  // Data scrambling ID
      pusch_pdu->ul_dmrs_scrambling_id,               // UL DMRS scrambling ID
      pusch_pdu->nrOfLayers,                          // Number of layers
      pusch_pdu->num_dmrs_cdm_grps_no_data,           // Number of DMRS CDM groups no data
      pusch_pdu->dmrs_ports,                          // DMRS ports
      pusch_pdu->ul_dmrs_symb_pos,                    // UL DMRS symbol position
      pusch_pdu->dmrs_config_type,                    // DMRS config type
      pusch_pdu->resource_alloc,                      // Resource allocation type
      pusch_pdu->rb_start,                            // Resource block start
      pusch_pdu->rb_size,                             // Resource block size
      pusch_pdu->frequency_hopping,                   // Frequency hopping
      pusch_pdu->vrb_to_prb_mapping,                  // VRB to PRB mapping
      pusch_pdu->start_symbol_index,                  // Start symbol index
      pusch_pdu->nr_of_symbols,                       // Number of symbols
      pusch_pdu->pusch_data.rv_index,                 // Redundancy version index
      pusch_pdu->pusch_data.harq_process_id,          // HARQ process ID
      pusch_pdu->pusch_data.new_data_indicator,       // New data indicator
      pusch_pdu->pusch_data.tb_size,                  // Transport block size
      pusch_pdu->dfts_ofdm.low_papr_group_number,     // Low PAPR group number
      pusch_pdu->dfts_ofdm.low_papr_sequence_number,  // Low PAPR sequence number
      pusch_pdu->pusch_ptrs.ptrs_freq_density,        // PTRS frequency density
      pusch_pdu->pusch_ptrs.ptrs_time_density,        // PTRS time density
      pusch_pdu->pusch_ptrs.num_ptrs_ports,           // Number of PTRS ports
      pusch_pdu->pusch_ptrs.ul_ptrs_power,            // UL PTRS power
      //pusch_pdu->absolute_delta_PUSCH,                // Absolute delta PUSCH
      //pusch_pdu->ldpcBaseGraph,                       // LDPC base graph
      pusch_pdu->pusch_data.num_cb,                   // Number of code blocks
      pusch_pdu->pdu_bit_map                          // PDU bitmap
  );

    


    if (false) continue; //configured_grant
