...
{
  LOG_I(NR_MAC, "process_addmod_bearers_cellGroupConfig\n");
  if (addmod == NULL)
    LOG_I(NR_MAC, "addmod->list.count ");
    return; /* nothing to do */
  LOG_I(NR_MAC, "addmod->list.count ");
  LOG_I(NR_MAC, "addmod->list.count %d\n", addmod->list.count);
  for (int i = 0; i < addmod->list.count; ++i) {
    LOG_D(NR_MAC, "addmod->list.array[%d]->logicalChannelIdentity %ld\n", i, addmod->list.array[i]->logicalChannelIdentity);
