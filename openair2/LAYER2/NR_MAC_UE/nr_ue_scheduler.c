...
fapi_nr_ul_config_request_pdu_t *lockGet_ul_config(NR_UE_MAC_INST_t *mac, frame_t frame_tx, int slot_tx, uint8_t pdu_type)
{
  ...
  NR_ConfiguredGrantConfig_t *configuredGrant = mac->current_UL_BWP->configuredGrantConfig;

    // Check if this is under a Configured Grant scenario
    if (configuredGrant) { //Configured_grant
        //LOG_I(NR_MAC, "Adjusting TDD configuration for Configured Grant\n");

        // Modify or override the TDD configuration based on Configured Grant
        // Example: Force the slot indicated by Configured Grant to be an uplink slot
        
          tdd_config->pattern1.nrofDownlinkSlots = 0; // Set the slot to UL
          tdd_config->pattern1.nrofUplinkSlots = 1;   // Force 1 UL slot
          tdd_config->pattern1.nrofUplinkSymbols = 0;
...
// - 6.3.1.7 of 38.211
int global_harq_id = 0;
int nr_config_pusch_pdu(NR_UE_MAC_INST_t *mac,
                        NR_tda_info_t *tda_info,
                        nfapi_nr_ue_pusch_pdu_t *pusch_config_pdu,
                        dci_pdu_rel15_t *dci,
                        NR_ConfiguredGrantConfig_t *configured_grant,
                        ...
                        const nr_dci_format_t dci_format)
{
  ...
  if (mac->ra.ra_state < nrRA_SUCCEEDED && mac->ra.ra_type == RA_2_STEP) {
    ...
    if (pusch_Config && pusch_Config->dmrs_UplinkForPUSCH_MappingTypeB &&
      ...
      if (!tp_enabled) {
  ...
  }else if (configured_grant) {
    
        pusch_config_pdu->bwp_start = configured_grant->rrc_ConfiguredUplinkGrant->frequencyDomainAllocation.buf[0];
        pusch_config_pdu->bwp_size = configured_grant->rrc_ConfiguredUplinkGrant->frequencyDomainAllocation.size;
        pusch_config_pdu->start_symbol_index = configured_grant->rrc_ConfiguredUplinkGrant->timeDomainAllocation;
        pusch_config_pdu->nr_of_symbols = 13;

        // 配置 MCS 和 TBS
        pusch_config_pdu->mcs_index = configured_grant->rrc_ConfiguredUplinkGrant->mcsAndTBS;
        
        // 這裡使用 Configured Grant 中的 DMRS 配置
        pusch_config_pdu->dmrs_config_type = pusch_dmrs_type1; // 默認配置 DMRS 類型 1
        pusch_config_pdu->num_dmrs_cdm_grps_no_data = 2; // 設置 DMRS CDM 群組數量

        // 如果需要變壓調整，這裡也可以從 CG 中讀取 power control 設定
        pusch_config_pdu->absolute_delta_PUSCH = configured_grant->p0_PUSCH_Alpha;

        // 如果有 precoding 配置，則處理 Transform Precoding
        if (configured_grant->transformPrecoder && *(configured_grant->transformPrecoder) == NR_PUSCH_Config__transformPrecoder_enabled) {
          pusch_config_pdu->transform_precoding = NR_PUSCH_Config__transformPrecoder_enabled;
          //pusch_config_pdu->pusch_identity = mac->physCellId;
          pusch_config_pdu->dfts_ofdm.low_papr_group_number = mac->physCellId % 30;
          pusch_config_pdu->dfts_ofdm.low_papr_sequence_number = 0;
        } else {
        ...
        /*
        // PTRS 設置，如果存在 phaseTrackingRS 的設定
        if (configured_grant->rrc_ConfiguredUplinkGrant->ext3->srs_ResourceSetId_r18) {
          bool valid_ptrs_setup = set_ul_ptrs_values(configured_grant->rrc_ConfiguredUplinkGrant->ext3->srs_ResourceSetId_r18->choice.setup,
                                                    pusch_config_pdu->rb_size,
                                                    pusch_config_pdu->mcs_index,
                                                    pusch_config_pdu->mcs_table,
                                                    &pusch_config_pdu->pusch_ptrs.ptrs_freq_density,
        ...
        */
        ...
        // 計算 TBS（transport block size）
        // 計算 TBS（Transport Block Size）
        int nb_dmrs_prb = ((pusch_config_pdu->dmrs_config_type == pusch_dmrs_type1) ? 6 : 4) * pusch_config_pdu->num_dmrs_cdm_grps_no_data;

        // 假設 tb_scaling 為 1（通常是 0 或 1），具體根據應用場景進行調整
        uint8_t tb_scaling = 1;
        ...
        pusch_config_pdu->bwp_start=0;
        pusch_config_pdu->bwp_size=106;
        pusch_config_pdu->start_symbol_index=10;
        pusch_config_pdu->nr_of_symbols=3;
        pusch_config_pdu->mcs_index=9;
        ...
        pusch_config_pdu->mcs_table=0;
        pusch_config_pdu->cyclic_prefix=0;
        pusch_config_pdu->data_scrambling_id=0;
        pusch_config_pdu->ul_dmrs_scrambling_id=0;
        pusch_config_pdu->subcarrier_spacing=1;
  ...
  if(false){ //Configured_grant
    //configured_grant && mac->state == UE_PERFORMING_RA_LESS
    configured_grant=mac->current_UL_BWP->configuredGrantConfig;
    pusch_config_pdu-> bwp_size= 106;
    pusch_config_pdu-> bwp_start= 0;
    pusch_config_pdu-> subcarrier_spacing= 1;
    pusch_config_pdu-> cyclic_prefix= 0;
    pusch_config_pdu-> target_code_rate= 6790;
    pusch_config_pdu-> qam_mod_order= 2;
    pusch_config_pdu-> mcs_index= 6;
    pusch_config_pdu-> mcs_table= 0;
    pusch_config_pdu-> transform_precoding= 1;
    pusch_config_pdu-> scid= 0;
    pusch_config_pdu-> data_scrambling_id= 1;
    pusch_config_pdu-> ul_dmrs_scrambling_id= 1;
    pusch_config_pdu-> nrOfLayers= 1;
    pusch_config_pdu-> num_dmrs_cdm_grps_no_data= 1;
    pusch_config_pdu-> dmrs_ports= 1;
    pusch_config_pdu-> ul_dmrs_symb_pos= 1057;
    pusch_config_pdu-> dmrs_config_type= 0;
    
    pusch_config_pdu-> resource_alloc= 1;


  // Example bit string for frequencyDomainAllocation: 11111000 (first 5 bits are 1, remaining bits are 0)
    uint8_t *freqDomainAlloc = configured_grant->rrc_ConfiguredUplinkGrant->frequencyDomainAllocation.buf;
    int num_bits = configured_grant->rrc_ConfiguredUplinkGrant->frequencyDomainAllocation.size * 8
                  - configured_grant->rrc_ConfiguredUplinkGrant->frequencyDomainAllocation.bits_unused;

    // Initialize rb_start and rb_size
    int rb_start = -1;
    int rb_size = 0;

    // Iterate over the bits to determine rb_start and rb_size
    for (int i = 0; i < num_bits; i++) {
        int bit = (freqDomainAlloc[i / 8] >> (7 - (i % 8))) & 0x01;  // Get the bit at position i
        if (bit == 1) {
            if (rb_start == -1) {
                rb_start = i;  // Set rb_start to the first occurrence of 1
            }
            rb_size++;  // Count the consecutive RBs (bits set to 1)
        } else if (rb_start != -1) {
            break;  // Stop counting when we find the first 0 after the start
        }
    }

    // Now assign rb_start and rb_size to PUSCH PDU
    pusch_config_pdu->rb_start = rb_start;
    pusch_config_pdu->rb_size = rb_size;

    //printf("PUSCH PDU: rb_start = %d, rb_size = %d\n", pusch_config_pdu->rb_start, pusch_config_pdu->rb_size);

    
    //pusch_config_pdu-> rb_start= (uint16_t)configured_grant->rrc_ConfiguredUplinkGrant->frequencyDomainAllocation.buf[0]; //0
    //pusch_config_pdu-> rb_size= (uint16_t)configured_grant->rrc_ConfiguredUplinkGrant->frequencyDomainAllocation.size; //5

    pusch_config_pdu-> frequency_hopping= 0;
    pusch_config_pdu-> vrb_to_prb_mapping= 0;

    pusch_config_pdu-> start_symbol_index= (uint8_t)configured_grant->rrc_ConfiguredUplinkGrant->timeDomainAllocation; //0
    pusch_config_pdu-> nr_of_symbols= (uint8_t)configured_grant->rrc_ConfiguredUplinkGrant->timeDomainOffset ;  //13
    ...
    pusch_config_pdu-> rb_size= 5;
    pusch_config_pdu-> start_symbol_index= 0;
    pusch_config_pdu-> nr_of_symbols= 13;
    */

    pusch_config_pdu-> pusch_data.rv_index= 0;
    pusch_config_pdu-> pusch_data.harq_process_id= (global_harq_id++)%16;
    pusch_config_pdu-> pusch_data.new_data_indicator= 1;
    pusch_config_pdu-> pusch_data.tb_size= 88;
    pusch_config_pdu-> tbslbrm= 0;
    pusch_config_pdu->dfts_ofdm.low_papr_group_number=0;     // Low PAPR group number
    pusch_config_pdu->dfts_ofdm.low_papr_sequence_number=0; // Low PAPR sequence number
    pusch_config_pdu->pusch_ptrs.ptrs_freq_density=0;        // PTRS frequency density
    pusch_config_pdu->pusch_ptrs.ptrs_time_density=0;        // PTRS time density
    pusch_config_pdu->pusch_ptrs.num_ptrs_ports=0;           // Number of PTRS ports
    pusch_config_pdu->pusch_ptrs.ul_ptrs_power=0;            // UL PTRS power
    pusch_config_pdu->absolute_delta_PUSCH=1;                // Absolute delta PUSCH
    pusch_config_pdu->ldpcBaseGraph=2;                       // LDPC base graph
    pusch_config_pdu->pusch_data.num_cb=0;                   // Number of code blocks
    pusch_config_pdu->pdu_bit_map=1;                          // PDU bitmap
    //LOG_I(NR_MAC,"global_harq_id=%d\n",global_harq_id);
  }
  //configured_grant
  if(mac->state == UE_PERFORMING_RA_LESS && (global_harq_id)%16 == 0){
        LOG_I(NR_MAC,"Change MAC state from UE_PERFORMING_RA_LESS to UE_CONNECTED\n");
...
bool is_configured_grant_slot(int current_SFN, int current_slot, int current_symbol, int periodicity, int timeReferenceSFN, int timeDomainOffset, int S, int numberOfSlotsPerFrame, int numberOfSymbolsPerSlot) {
    // Calculate the left side of the equation
    int current_time = (current_SFN * numberOfSlotsPerFrame * numberOfSymbolsPerSlot) + (current_slot * numberOfSymbolsPerSlot) + current_symbol;

    // Calculate the right side of the equation for the Nth periodicity
    int base_time = (timeReferenceSFN * numberOfSlotsPerFrame * numberOfSymbolsPerSlot) + (timeDomainOffset * numberOfSymbolsPerSlot) + S;
    
    // Print debug information
    // LOG_I(NR_MAC, "Debugging is_configured_grant_slot: \n");
    //LOG_I(NR_MAC, "current_SFN: %d, current_slot: %d, current_symbol: %d\n", current_SFN, current_slot, current_symbol);
    // LOG_I(NR_MAC, "periodicity: %d, timeReferenceSFN: %d, timeDomainOffset: %d, S: %d\n", periodicity, timeReferenceSFN, timeDomainOffset, S);
    // LOG_I(NR_MAC, "numberOfSlotsPerFrame: %d, numberOfSymbolsPerSlot: %d\n", numberOfSlotsPerFrame, numberOfSymbolsPerSlot);
    // LOG_I(NR_MAC, "current_time: %d, base_time: %d\n", current_time, base_time);
    
    // Check if the current time is a valid configured grant slot
    bool result = ((current_time - base_time) % (periodicity * numberOfSymbolsPerSlot * numberOfSlotsPerFrame)) == 0;
    
    //LOG_I(NR_MAC, "Result: %s\n", result ? "true" : "false");
    
    return result;
}

static int nr_ue_process_configured_grant(NR_UE_MAC_INST_t *mac,
                                          frame_t frame,
                                          int slot,
                                          NR_ConfiguredGrantConfig_t *configured_grant,
                                          uint16_t rnti) {
    //LOG_I(NR_MAC, "Processing Configured Grant PUSCH\n");
    const NR_UE_UL_BWP_t *current_UL_BWP = mac->current_UL_BWP;
    long period=(int)configured_grant->periodicity;
    bool is_valid_slot = is_configured_grant_slot(frame, slot, 0, period, 20,
                                                    7, 0,
                                                    nr_slots_per_frame[current_UL_BWP->scs], 14 /*symbols per slot*/);
    if (!is_valid_slot) return -1;

    // 當發現有效的 Configured Grant 傳輸槽位後，計算傳輸的 frame 和 slot
    frame_t frame_tx = frame;
    int slot_tx = slot+1;

    LOG_D(NR_MAC, "Apply configured grant UL transmission in [%04d.%02d] for %x\n",
            frame_tx, slot_tx,rnti);
  

    // Retrieve and lock the PUSCH configuration PDU
    fapi_nr_ul_config_request_pdu_t *pdu = lockGet_ul_config(mac, frame_tx, slot_tx, FAPI_NR_UL_CONFIG_TYPE_PUSCH);
    if (!pdu) {
        LOG_E(MAC, "Failed to allocate UL config PDU for Configured Grant\n");
        return -1;
    }

    // Configure the PUSCH PDU using the configured grant
    NR_tda_info_t tda_info = {};
    int ret = nr_config_pusch_pdu(mac, &tda_info, &pdu->pusch_config_pdu, NULL, configured_grant, NULL,NULL, rnti, NR_SearchSpace__searchSpaceType_PR_ue_Specific, NR_UL_DCI_FORMAT_0_0);
    if (ret != 0) {
        remove_ul_config_last_item(pdu);
        LOG_E(MAC, "PUSCH configuration failed for Configured Grant\n");
    }

    release_ul_config(pdu, false);
    /*
    if(mac->state == UE_PERFORMING_RA_LESS)
        mac->state = UE_CONNECTED;
    */
    return ret;
}
...
void nr_ue_ul_scheduler(NR_UE_MAC_INST_t *mac, nr_uplink_indication_t *ul_info)
{
  ...
  if(mac->state > UE_NOT_SYNC && mac->state < UE_CONNECTED || mac->state == UE_PERFORMING_RA_LESS) {

    //rach-based mac->current_UL_BWP->configuredGrantConfig
    if(mac->current_UL_BWP->configuredGrantConfig){
      //nr_ue_get_rach(mac, cc_id, frame_tx, gNB_index, slot_tx);
      //nr_ue_prach_scheduler(mac, frame_tx, slot_tx); 
      nr_ue_handle_rach_less(mac, frame_tx, gNB_index, slot_tx);
      //configured_grant
      //nr_ue_process_configured_grant(mac, frame_tx, slot_tx, mac->current_UL_BWP->configuredGrantConfig,mac->crnti);
      nr_mac_rrc_ra_ind(mac->ue_id, frame_tx, true);
