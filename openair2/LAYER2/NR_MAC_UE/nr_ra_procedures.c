                            ...
                            NR_RACH_ConfigDedicated_t *rach_ConfigDedicated)
{
  ...
  if (rach_ConfigDedicated) {
    ...
    LOG_D(MAC, "DL pathloss is %d dB\n", dl_pathloss);
    ssb_rach_config(ra, prach_resources, nr_rach_ConfigCommon);
    ra_preambles_config(prach_resources, mac, dl_pathloss);
    LOG_I(MAC, "[RAPROC] - Selected RA preamble index %d for contention-based random access procedure... \n", ra->ra_PreambleIndex);
  }
...
void nr_ue_handle_rach_less(NR_UE_MAC_INST_t *mac, frame_t frame, uint8_t gNB_id, int nr_slot_tx){
  mac->state = UE_PERFORMING_RA_LESS;
...
void nr_ra_succeeded(NR_UE_MAC_INST_t *mac, const uint8_t gNB_index, const frame_t frame, const int slot)
{
  ...
  } else if (mac->state == UE_PERFORMING_RA_LESS){
    LOG_I(MAC, "[UE %d][%d.%d][RAPROC] RA-less procedure succeeded. CFRA: RAR successfully received.\n", mac->ue_id, frame, slot);
  }
  else {
    LOG_A(MAC,
