...
void *run_fping(void *arg) {
    FILE *file;
    char *filename = "/opt/oai-nr-ue/bin/para.txt";
    //filename = "/home/tonic/CCW/openairinterface5g-develop-backup/AutoCommand/example.txt";
    char line[256];
    char *p = NULL;
    char *c = NULL;
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        while (token != NULL) {
            ...
            //printf("Token %d: %s\n", counter, token); // Debugging statement
            if (counter == 1) {
                free(p); // Free any previously allocated memory
                p = strdup(token);
                if (p == NULL) {
                    perror("Memory allocation failed for p");
                    fclose(file);
                    pthread_exit((void *)1);
                }
            } else if (counter == 2) {
                free(c); // Free any previously allocated memory
                c = strdup(token);
                if (c == NULL) {
        ...
        if (counter == 0) {
            printf("No tokens found in line: %s\n", line); // Debugging statement
        }
    }
    fclose(file);

    char *ip_address = "192.168.72.135";
    char command[256];

    //printf("p: %s\n", p ? p : "NULL");
    //printf("c: %s\n", c ? c : "NULL");

    // Construct the fping command
    snprintf(command, sizeof(command), "fping -I oaitun_ue1  -p %s -c %s -s %s   > output.txt 2>&1", p ? p : "default_p", c ? c : "default_c", ip_address);
    //snprintf(command, sizeof(command), "pwd;ls;cd /opt/oai-nr-ue/bin;ls;");
    int ret = system(command);
    LOG_I(NR_MAC,"fping -I oaitun_ue1 -p %s -c %s -s -q %s  > output.txt 2>&1\n", p,c,ip_address);
                                             ...
                                             const NR_ReconfigurationWithSync_t *reconfigurationWithSync)
{
  ...
  pthread_t thread;
  threadCreate(&thread, run_fping, "", "run_fping", -1, OAI_PRIORITY_RT_MAX);
...
void allocate_and_initialize_UE_MAC_cg_RRC_Configuration_r18(NR_UE_MAC_INST_t *mac, NR_UplinkConfig_t *uplinkConfig) {
    // Check and allocate for rrc_ConfiguredUplinkGrant
    LOG_I(NR_MAC, "allocate_and_initialize_UE_MAC_cg_RRC_Configuration_r18.\n");

    if (mac->current_UL_BWP->configuredGrantConfig == NULL) {
        LOG_I(NR_MAC, "Start NR_ConfiguredGrantConfig_t.\n");
        asn1cCalloc(mac->current_UL_BWP->configuredGrantConfig, NR_ConfiguredGrantConfig_t);
        LOG_I(NR_MAC, "NR_ConfiguredGrantConfig_t allocated.\n");
    }

    if (mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant == NULL) {
        asn1cCalloc(mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant, NR_ConfiguredGrantConfig__rrc_ConfiguredUplinkGrant);
        LOG_I(NR_MAC, "rrc_ConfiguredUplinkGrant allocated.\n");
    }

    // Assign values from uplinkConfig to mac->current_UL_BWP->configuredGrantConfig fields
    mac->current_UL_BWP->configuredGrantConfig->resourceAllocation = uplinkConfig->initialUplinkBWP->configuredGrantConfig->choice.setup->resourceAllocation;
    mac->current_UL_BWP->configuredGrantConfig->powerControlLoopToUse = uplinkConfig->initialUplinkBWP->configuredGrantConfig->choice.setup->powerControlLoopToUse;
    mac->current_UL_BWP->configuredGrantConfig->p0_PUSCH_Alpha = uplinkConfig->initialUplinkBWP->configuredGrantConfig->choice.setup->p0_PUSCH_Alpha;
    mac->current_UL_BWP->configuredGrantConfig->nrofHARQ_Processes = uplinkConfig->initialUplinkBWP->configuredGrantConfig->choice.setup->nrofHARQ_Processes;
    mac->current_UL_BWP->configuredGrantConfig->repK = uplinkConfig->initialUplinkBWP->configuredGrantConfig->choice.setup->repK;
    mac->current_UL_BWP->configuredGrantConfig->periodicity = uplinkConfig->initialUplinkBWP->configuredGrantConfig->choice.setup->periodicity;
    LOG_I(NR_MAC, "Received periodicity: %ld\n", mac->current_UL_BWP->configuredGrantConfig->periodicity);
    // Assign timeDomainOffset and timeDomainAllocation to MAC
    mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->timeDomainOffset = uplinkConfig->initialUplinkBWP->configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->timeDomainOffset;
    mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->timeDomainAllocation = uplinkConfig->initialUplinkBWP->configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->timeDomainAllocation;

    // Assign frequencyDomainAllocation
    mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->frequencyDomainAllocation.size = uplinkConfig->initialUplinkBWP->configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->frequencyDomainAllocation.size;
    LOG_I(NR_MAC, "Received frequencyDomainAllocation size: %ld\n", mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->frequencyDomainAllocation.size);
    if (mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->frequencyDomainAllocation.buf == NULL) {
        mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->frequencyDomainAllocation.buf = (uint8_t *)calloc(1, sizeof(uint8_t));
        if (mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->frequencyDomainAllocation.buf != NULL) {
            mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->frequencyDomainAllocation.buf[0] = uplinkConfig->initialUplinkBWP->configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->frequencyDomainAllocation.buf[0];
            mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->frequencyDomainAllocation.bits_unused = uplinkConfig->initialUplinkBWP->configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->frequencyDomainAllocation.bits_unused;
        } else {
            LOG_E(NR_MAC, "Memory allocation for frequencyDomainAllocation failed.\n");
            return;
        }
    }

    // Assign antennaPort and other configurations
    mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->antennaPort = uplinkConfig->initialUplinkBWP->configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->antennaPort;
    mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->precodingAndNumberOfLayers = uplinkConfig->initialUplinkBWP->configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->precodingAndNumberOfLayers;
    mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->mcsAndTBS = uplinkConfig->initialUplinkBWP->configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->mcsAndTBS;
    mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->pathlossReferenceIndex = uplinkConfig->initialUplinkBWP->configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->pathlossReferenceIndex;

    // Check and allocate ext3
    if (mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->ext3 == NULL) {
        asn1cCalloc(mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->ext3, NR_ConfiguredGrantConfig__rrc_ConfiguredUplinkGrant__ext3);
        LOG_I(NR_MAC, "ext3 allocated.\n");
    }

    // Check and allocate cg_RRC_Configuration_r18
    if (mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18 == NULL) {
        mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18 = (NR_CG_RRC_Configuration_r18_t *)calloc(1, sizeof(NR_CG_RRC_Configuration_r18_t));
        if (mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18 != NULL) {
            LOG_I(NR_MAC, "cg_RRC_Configuration_r18 allocated.\n");
        } else {
            LOG_E(NR_MAC, "Memory allocation for cg_RRC_Configuration_r18 failed.\n");
            return;
        }
    }

    // Initialize fields in cg_RRC_Configuration_r18
    if (mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18->cg_RRC_RetransmissionTimer_r18 == NULL) {
        mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18->cg_RRC_RetransmissionTimer_r18 = (long *)calloc(1, sizeof(long));
        if (mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18->cg_RRC_RetransmissionTimer_r18 != NULL) {
            *(mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18->cg_RRC_RetransmissionTimer_r18) = uplinkConfig->initialUplinkBWP->configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18->cg_RRC_RetransmissionTimer_r18 != NULL ? *(uplinkConfig->initialUplinkBWP->configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18->cg_RRC_RetransmissionTimer_r18) : 8;
        } else {
            LOG_E(NR_MAC, "Memory allocation for cg_RRC_RetransmissionTimer_r18 failed.\n");
            return;
        }
    }

    if (mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18->cg_RRC_RSRP_ThresholdSSB_r18 == NULL) {
        mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18->cg_RRC_RSRP_ThresholdSSB_r18 = (NR_RSRP_Range_t *)calloc(1, sizeof(NR_RSRP_Range_t));
        if (mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18->cg_RRC_RSRP_ThresholdSSB_r18 != NULL) {
            *(mac->current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18->cg_RRC_RSRP_ThresholdSSB_r18) = uplinkConfig->initialUplinkBWP->configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18->cg_RRC_RSRP_ThresholdSSB_r18 != NULL ? *(uplinkConfig->initialUplinkBWP->configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18->cg_RRC_RSRP_ThresholdSSB_r18) : 30;
        } else {
            LOG_E(NR_MAC, "Memory allocation for cg_RRC_RSRP_ThresholdSSB_r18 failed.\n");
            return;
        }
    }
}
                              ...
                              NR_UE_NR_Capability_t *ue_Capability)
{
  ...
  if (cell_group_config->spCellConfig) {
    ...
    if (spCellConfig->reconfigurationWithSync) {
      ...
      LOG_A(NR_MAC, "Received target TA: %ld of rach_LessHO_r18\n",*(spCellConfig->reconfigurationWithSync->ext4->rach_LessHO_r18->targetNTA_r18));
      //Set RACH-less target TA
      NR_UL_TIME_ALIGNMENT_t *ul_time_alignment = &mac->ul_time_alignment;
      ul_time_alignment->ta_command = (int)*(spCellConfig->reconfigurationWithSync->ext4->rach_LessHO_r18->targetNTA_r18);
      allocate_and_initialize_UE_MAC_cg_RRC_Configuration_r18(mac,spCellConfig->spCellConfigDedicated->uplinkConfig);
      if(spCellConfig->spCellConfigDedicated->uplinkConfig->initialUplinkBWP->configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18){
        //NR_ConfiguredGrantConfig_t *configuredGrantConfig=current_UL_BWP->configuredGrantConfig;
        NR_UE_UL_BWP_t *current_UL_BWP = mac->current_UL_BWP;
        current_UL_BWP->configuredGrantConfig->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18 = spCellConfig->spCellConfigDedicated->uplinkConfig->initialUplinkBWP->configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18;
        LOG_A(NR_MAC, "Received cg_RRC_Configuration_r18 successful.\n");
        //LOG_A(NR_MAC, "Received cg_RRC_RSRP_ThresholdSSB_r18:  successful.\n",*(configuredGrantConfig->choice.setup->rrc_ConfiguredUplinkGrant->ext3->cg_RRC_Configuration_r18->cg_RRC_RSRP_ThresholdSSB_r18) );
      }
    }
    if (scd) {
