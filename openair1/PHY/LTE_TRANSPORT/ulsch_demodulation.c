...
/* Check this modification w.r.t to new PUSCH modifications
  ...
  */   
  ...
  uint16_t nb_re = ulsch[UE_id]->harq_processes[harq_pid]->nb_rb * frame_parms->symbols_per_tti ? 
                   ulsch[UE_id]->harq_processes[harq_pid]->nb_rb * 12 : 
                   frame_parms->N_RB_UL * 12; 
  FILE *constel_fd = fopen("constellation.txt","a");
  if (!constel_fd) {
    LOG_E(PHY, "Could not open constellation.txt for writing\n");
  } else {
    // For each symbol l in this subframe, write all REs
    for (int l_sym = 0;
        l_sym < (frame_parms->symbols_per_tti - ulsch[UE_id]->harq_processes[harq_pid]->srs_active);
        l_sym++) {

      // Skip pilot symbols if desired
      if (((frame_parms->Ncp == 0) && ((l_sym==3) || (l_sym==10))) ||
          ((frame_parms->Ncp == 1) && ((l_sym==2) || (l_sym==8)))) {
        continue;
      }

      // Write each complex symbol (I Q) on its own line
      for (int re = 0; re < nb_re; re++) {
        complexd sym = pusch_vars->rxdataF_comp[l_sym][re];
        fprintf(constel_fd, "%f %f\n", creal(sym), cimag(sym));
      }
    }
